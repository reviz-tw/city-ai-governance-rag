...
- [x] 重新檢查 Vertex AI Search 的資料結構、語意檢索設定及多語 embedding 是否真的支援預期語種；不要只用生成階段的翻譯能力代替跨語檢索驗證。
- [x] 更新 FE API、MCP tool schema、後端 schema、prompt 與文件，避免同名欄位具有不同含義。

#### 驗收條件

- [x] 中文提問能檢索到英文／日文等原文，並以中文回答及引用原文來源。
- [x] 明確指定 `source_languages=["en"]` 時，只回傳英文來源。
...
- 採用 [Chunk-as-Document](docs/chunk-as-document.md)：每個人工切片以獨立 TXT 與標準 Document 匯入 Vertex AI Search v1，不使用 BYO 巢狀 chunks 或 pgvector。發布約等候 10～30 分鐘，依 Vertex 佇列及驗證結果生效。
...
- [x] 驗證同一文件的 chunks 更新、合併／拆分及重新匯入行為，確保舊片段不殘留或與新版重複；通過後才開發完整編輯流程。
...
- [x] 草稿修改不影響 FE／MCP 的正式回答；發布並確認索引完成後，兩個入口都能檢索修正後內容。
- [x] Admin 顯示的已發布 chunks 與實際搜尋結果一致，不再以本地臨時預覽冒充索引狀態。
- [x] 合併／拆分後不出現舊片段重複命中，引用仍能對應原始文件與來源範圍。
...
- [x] 在 staging／dev Cloud Run 完成 smoke test 後才更新 production；記錄實際模型 ID、SDK 版本及 rollback 方法。
...
- [x] Vertex AI Search 的搜尋與文件匯入流程通過整合測試。
