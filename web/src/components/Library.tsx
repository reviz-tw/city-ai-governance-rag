...
export function Library({editor, onClose, onRead}: {editor: boolean; onClose: () => void; onRead: (id:string)=>void}) {
  ...
  if(job) return <JobView id={job} onClose={()=>{setJob(''); void refresh();}}/>;
  return <section className="workspace-panel"><div className="flex justify-between"><h2>來源文件庫</h2><button onClick={onClose}>關閉</button></div>
    ...
    {doc && editor && <div><h3>{doc.title} · 草稿修訂 {doc.draft_revision}</h3><p>索引狀態：{doc.index_status}；發布版本：{doc.published_version}</p>
      <div className="parallel-text"><div><h3>原始擷取</h3>{doc.blocks?.map((b:any)=><p key={b.id} dir="auto">{b.id} · {b.text}</p>)}</div><div><h3>切片草稿</h3>{doc.draft?.map((c:any,i:number)=><div key={c.id}><small>{c.id} · {c.refs.map((r:any)=>r.block_id).join(', ')}</small><textarea value={c.content} dir="auto" onChange={e=>setDoc({...doc,draft:doc.draft.map((v:any,j:number)=>i===j?{...v,content:e.target.value}:v)})}/>
        ...
        <button onClick={()=>{const position=c.content.indexOf('\n',Math.floor(c.content.length/2));const cut=position>0?position:Math.floor(c.content.length/2);setDoc({...doc,draft:[...doc.draft.slice(0,i),{...c,content:c.content.slice(0,cut)},{...c,id:crypto.randomUUID(),content:c.content.slice(cut)},...doc.draft.slice(i+1)].map((v:any,order:number)=>({...v,order}))});}}>拆分為兩片</button>
      </div>)}</div></div>
      ...
      {!doc.indexing_enabled&&<p>搜尋索引尚未啟用；草稿可以儲存，完成索引設定後才能發布。</p>}
