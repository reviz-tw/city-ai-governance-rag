...
export function MCPAccess({onClose}:{onClose:()=>void}) {
  ...
  return <section className="workspace-panel"><div className="flex justify-between"><h2>MCP 連線</h2><button onClick={onClose}>關閉</button></div>
    <p>為你自己的 MCP 用戶端建立一小時憑證。權限與目前帳號相同，僅能用於 MCP；不要分享或貼進對話。新憑證會撤銷上一個。</p>
    <p>端點：<code>{location.origin}/mcp</code></p>
    <button className="btn" onClick={async()=>{try {setError('');setCredential(await api('/api/auth/mcp-token',{method:'POST'}));}catch(e:any){setError(e.message);}}}>建立個人短期憑證</button>
