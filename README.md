...
切片編輯器一次顯示一片，提供加大的文字框、上一片／下一片與下拉選擇；原文對照可展開，僅顯示目前切片的來源段落。新 PDF 的初始草稿與重新切片會整理可辨識的排版斷行，保留原始擷取、頁碼與原文範圍。既有草稿可按「整理所有切片換行」預覽並撤回，再儲存、查看差異及送出索引；不會自動覆寫既有草稿或發布。整理採固定規則，不呼叫 LLM；保留段落、條列、表格與程式碼，無法確定的換行仍交由編輯者檢查。
